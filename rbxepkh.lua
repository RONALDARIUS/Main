...
--camera
dd:addToggle("Esp Camera", nil, function(value)
    if value then
...
ot:addToggle("Esp Vault", nil, function(value)
    if value then
        ...
        for i,v in pairs(game:GetService("Workspace").Level.Geometry:GetDescendants()) do
            if v.Name == "VaultDoor" then
                if v.Vault:FindFirstChild("ESP") then
                    v.Vault.ESP:Destroy()
                end
            end
        end
...
--location
ot:addToggle("Esp Security Location", nil, function(value)
    if value then
